const { abs, min, max, round, pow, random, sin, cos, PI } = Math;
...
class RGBtoHSL {
    ...
    public hsl() {
        ...
        if (maxV === minV) h = s = 0;
            ...
            let d = maxV - minV;
            s = l > .5 ? d / (2 - maxV - minV) : d / (maxV + minV);
            switch(maxV) {
    ...
    private spsa(A: number, a: number[], c: number, values: number[], iters: number) {
        ...
        for (let k = 0; k < iters; k++) {
            let ck = c / pow(k + 1, gamma);

            for (let i = 0; i < 6; i++) {
                deltas[i] = random() > .5 ? 1 : -1;
    ...
    private loss(filters: number[]) {
        ...
        return abs(color.r - this.target.r) + abs(colorHSL.h - this.targetHSL.h)
             + abs(color.g - this.target.g) + abs(colorHSL.s - this.targetHSL.s)
             + abs(color.b - this.target.b) + abs(colorHSL.l - this.targetHSL.l);
    }

    private toString(filters: number[]) {
        const format = (idx: number, multiplier = 1) => round(filters[idx] * multiplier);
